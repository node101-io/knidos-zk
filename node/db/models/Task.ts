...
export interface TaskModel extends Model<TaskInterface> {
  createTask: (
    ...
    callback: (err: string | null, task: TaskInterface | null) => any,
  ) => any;

  findTasksByPipelineId: (
    body: { pipelineId: Types.ObjectId },
    callback: (err: string | null, tasks: TaskInterface[] | null) => any,
  ) => any;
  updateTaskStatus: (
    body: {
      taskId: string;
...
TaskSchema.statics.createTask = function (
  body: Parameters<TaskModel['createTask']>[0],
  callback: Parameters<TaskModel['createTask']>[1],
) {
...
TaskSchema.statics.findTasksByPipelineId = function (
  body: Parameters<TaskModel['findTasksByPipelineId']>[0],
  callback: Parameters<TaskModel['findTasksByPipelineId']>[1],
) {
...
const Task =
  ...
  mongoose.model<TaskInterface, TaskModel>('Task', TaskSchema);
