...
Capacity note: a window can hold at most **16 fills**. The binding limit is the Noir JSON parser's token budget (`JSON8kb`, 1024 tokens; a Hyperliquid fill spends ~60), which is hit before the 8 KB byte buffer — measured with real mainnet fills: 16 parse, 17 fail with `MaxNumTokens limit exceeded`. The zkTLS worker fetches the window once before buying a Primus attestation and fails the task permanently if it is over either limit, so an unprovable window costs no gas; a window the circuit rejects for any other reason (for example the PnL threshold) fails at the noir stage without retries. Shortening `ZKTLS_WINDOW_MINUTES` is the lever if windows overflow.
